...
async def create_lead(
    ...
    # direct check for duplicate phone number with specific error message
    ...
    # timezone-aware UTC timestamps for database consistency
...
async def get_leads(
    ...
    # Enforced backend RBAC data isolation. (A sales executive should only see statistics for leads they are permitted to access)
...
async def get_lead(
    ...
    # Enforce RBAC: Sales executives can only view assigned leads
...
# PATCH /{lead_id} endpoint for updates and assignments
...
async def update_lead(
    ...
    # Enforce RBAC: Sales executives can only update their assigned leads
    if current_user["role"] == "sales_executive" and existing_lead.get(
        "assigned_to"
    ...
    # Enforce assignment rules: Only managers or owners can assign leads
    if "assigned_to" in update_data and current_user["role"] == "sales_executive":
        raise HTTPException(
    ...
    # Automatically create activity records for status and assignment changes
        ...
        activities_to_insert.append(
            {
                ...
                "description": f"Lead assigned to {update_data['assigned_to']}",
                "created_by": str(current_user["_id"]),
                "created_at": now,
            }
        )
...
# DELETE /{lead_id} endpoint restricted to owners
@router.delete("/{lead_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_lead(
    lead_id: str,
    # RBAC: Only Owners can delete leads
    current_user: dict = Depends(RoleChecker([UserRole.OWNER])),
    db=Depends(get_database),
):
    if not ObjectId.is_valid(lead_id):
